...
# exit
w2 = Button(master, text="重置", command=(lambda x=ALL: reload_hanoi()), width=12, height=2,
            bg='red');  # “重置”按钮绑定reload_hanoi()
w2.pack(padx=1, pady=1, side=LEFT);
w6 = Button(master, text="单步执行", command=(lambda x=ALL: StepByStep()), width=12, height=2,
            bg='red')  # 创建“单步执行”按钮并绑定函数 StepByStep()
w6.pack(padx=1, pady=1, side=LEFT);
w3 = Button(master, text="自动移动", command=(lambda x=ALL: redo_hanoi()), width=12, height=2,
            bg='red');  # “自动移动”按钮绑定redo_hanoi()
